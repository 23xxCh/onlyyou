...
**告诉你的 AI Agent（飞书 Aily / Claude Code / OpenClaw / Cursor）：**

```
帮我安装：https://github.com/23xxCh/onlyyou
```
...
cp -R onlyyou ~/.openclaw/skills/feishu-ecommerce
```

**更新：**

```bash
cd ~/.openclaw/skills/feishu-ecommerce && git pull
```
...
飞书电商助手是一个**已经养好**的 AI 技能包，安装后你的 Agent 就具备电商+飞书的全套能力。
...
| 电商案例库 | ✅ 开箱即用 | 无需配置 |
| AI 生图/视频 | ⚙️ 需配置 | Waninter API Key |
| 邮件监听 | ⚙️ 需配置 | IMAP 邮箱信息 |
| 互联网搜索 | ⚙️ 需配置 | `pip install agent-reach` |
| 小红书 | ⚙️ 需配置 | 浏览器 Cookie |
